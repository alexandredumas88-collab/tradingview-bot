#!/usr/bin/env python3
"""Check ALL Chrome cookies for tradingview and google domains."""
...
# Look for ALL cookies from tradingview + google domains
cur.execute(
    "SELECT host_key, name, encrypted_value, expires_utc, is_httponly, is_secure "
    "FROM cookies "
    "WHERE host_key LIKE '%tradingview%' OR host_key LIKE '%google%' "
...
print(f"Total cookies (TV + Google): {len(rows)}")
print()
for r in rows:
    v = decrypt(bytes(r['encrypted_value']))
    expires = r['expires_utc']
    exp_str = 'session' if expires == 0 else 'persistent'
    h_flag = 'H' if r['is_httponly'] else ' '
    print(f"  [{exp_str:9s}] [{h_flag}] {r['host_key']:45s}  {r['name']:40s}  = {v[:50]!r}")
