#!/usr/bin/env python3
"""
Stop-loss sweep on SP500 and NAS100.
Strategy: BB+RSI level entry (rsi < 35) + hard stop-loss.
Grid: 2 assets x 3 BB lengths x 4 SL sizes = 24 combos.
...
BB_LENS  = [10, 20, 30]
SL_PCTS  = [0.015, 0.020, 0.025, 0.030]   # 1.5 % … 3.0 %
ASSETS   = ['SP500', 'NAS100']

_here       = os.path.dirname(__file__)
DATA_CACHE  = os.path.join(_here, 'pydata')
REPORT_OUT  = os.path.join(_here, 'pysl-report.html')
RESULTS_OUT = os.path.join(_here, 'pysl-results.json')
...
class BBRSIWithSL(Strategy):
    bb_period  = 20
    rsi_period = 7
    rsi_lo     = 35
    sl_pct     = 0.02    # 2 % hard stop-loss below entry
    ...
    def next(self):
        ...
        if not self.position:
            if self.data.Close[-1] < self.bb_lo[-1] and self.rsi[-1] < self.rsi_lo:
...
def run_one(asset, df, bb, sl_pct):
    BBRSIWithSL.bb_period = bb
    BBRSIWithSL.sl_pct    = sl_pct

    try:
        bt    = Backtest(df, BBRSIWithSL, cash=INITIAL_CAPITAL, commission=COMMISSION, exclusive_orders=True)
        stats = bt.run()
        ...
        sym    = 'OK' if ok else 'X '
        print(f'  {sym}  {asset:7s} BB{bb:2d}  SL {sl_pct*100:.1f}%   '
              f'{trades:4d}tr ({tpy:.0f}/yr)  PF {pf:.3f}  DD {dd:.1f}%  Ret {ret:+.1f}%{flag_s}')

        return dict(
            asset=asset, bb=bb, sl_pct=sl_pct,
        ...
        print(f'  X   {asset:7s} BB{bb:2d}  SL {sl_pct*100:.1f}%   ERROR: {e}')
        return dict(asset=asset, bb=bb, sl_pct=sl_pct, error=str(e), ok=False)
...
def generate_report(results):
  ...
  <div class="clabel">BB({r["bb"]}) · SL {r["sl_pct"]*100:.1f}%</div>
  <div class="mgrid">
  ...
  <div class="badges">
    ...
    # ── Heatmaps ──────────────────────────────────────────────────────────────
    def heatmap(asset):
        lookup = {(r['bb'], r['sl_pct']): r for r in valid if r['asset'] == asset}
        head   = '<tr><th>SL \\ BB</th>' + ''.join(f'<th>BB({b})</th>' for b in BB_LENS) + '</tr>'
        body   = ''
        for sl in SL_PCTS:
            body += f'<tr><th class="sl-th">SL {sl*100:.1f}%</th>'
            for bb in BB_LENS:
                r = lookup.get((bb, sl))
                if not r:
                    body += '<td class="na">—</td>'
    ...
    for i, r in enumerate(ranked):
        ...
        tbl_rows += (f'<tr style="{bg}">'
                     ...
                     f'<td>BB({r["bb"]})</td>'
                     f'<td>SL {r["sl_pct"]*100:.1f}%</td>'
                     f'<td>{"FTMO" if r["ok"] else "—"}</td>'
                     f'<td style="color:{"#4caf50" if r["mt"] else "#f44336"}">{r["tpy"]} ({r["trades"]} total)</td>'
    ...
    if qualified:
        ...
        rec = (f'<div class="rec good">'
               f'<b>Recommended: {best["asset"]} BB({best["bb"]}) SL {best["sl_pct"]*100:.1f}%</b><br>'
               f'PF {best["pf"]} &nbsp;·&nbsp; {best["tpy"]}/yr &nbsp;·&nbsp; '
               f'DD {best["dd"]}% &nbsp;·&nbsp; Return {best["ret"]}% &nbsp;·&nbsp; '
               f'Win Rate {best["wr"]}% &nbsp;·&nbsp; Sharpe {best["sharpe"]}'
               f'</div>')
    else:
        # Show the two closest: highest PF and lowest DD gap
        best_pf = max(valid, key=lambda r: r['pf'], default={})
        # combo where sum of normalised gaps is smallest
        def gap(r):
            g_t = max(0, (TARGETS['min_trades_yr'] - r['tpy']) / TARGETS['min_trades_yr'])
            g_p = max(0, (TARGETS['min_pf']        - r['pf'])  / TARGETS['min_pf'])
            g_d = max(0, (r['dd'] - TARGETS['max_dd_pct'])     / TARGETS['max_dd_pct'])
            return g_t + g_p + g_d
        closest = min(valid, key=gap, default={})
        rec = (f'<div class="rec warn">'
               f'<b>No combo qualifies yet.</b><br><br>'
               f'Closest overall: <b>{closest.get("asset")} BB({closest.get("bb")}) SL {closest.get("sl_pct",0)*100:.1f}%</b>'
               f' — PF {closest.get("pf","?")} · {closest.get("tpy","?")}/yr · DD {closest.get("dd","?")}%<br>'
               f'Highest PF: <b>{best_pf.get("asset")} BB({best_pf.get("bb")}) SL {best_pf.get("sl_pct",0)*100:.1f}%</b>'
               f' — PF {best_pf.get("pf","?")} · {best_pf.get("tpy","?")}/yr · DD {best_pf.get("dd","?")}%<br><br>'
               f'If DD is the only blocker, consider adding a daily max-loss circuit-breaker (skip trading after 2 consecutive SL hits in one day). '
               f'If PF is the only blocker, add a 50-EMA trend filter (only buy when price &gt; EMA50).'
               f'</div>')
...
<head>
<meta charset="UTF-8">
<title>SL Sweep Report — SP500 &amp; NAS100</title>
<style>
...
table.heat{{border-collapse:collapse;min-width:500px}}
table.heat th{{background:#141414;color:#aaa;padding:10px 16px;border:1px solid #2a2a2a;text-align:center;font-size:12px}}
.sl-th{{text-align:left!important;color:#f0b429;font-weight:bold;white-space:nowrap}}
td.hit{{background:#0e2e0e;border:1px solid #2a4a2a;text-align:center;padding:10px 14px;min-width:110px}}
td.miss{{background:#1a1a1a;border:1px solid #2a2a2a;text-align:center;padding:10px 14px;min-width:110px}}
...
<body>
<h1>Stop-Loss Sweep — SP500 &amp; NAS100</h1>
<p class="sub">BB+RSI level entry (rsi &lt; 35) · BB 10/20/30 · SL 1.5/2.0/2.5/3.0% · 2Y hourly · Commission 0.01% · Generated {now}</p>
...
<h2>All Combos — Ranked by PF</h2>
<div class="cards">{cards_html}</div>

<h2>Full Comparison Table</h2>
<div class="twrap">
<table>
  <thead><tr><th>Asset</th><th>BB</th><th>SL</th><th>FTMO</th><th>Trades/yr</th><th>PF</th><th>Max DD</th><th>Return</th><th>Win Rate</th><th>Sharpe</th></tr></thead>
  <tbody>{tbl_rows}</tbody>
</table>
</div>

<h2>SP500 — Heatmap (green = all 3 targets met)</h2>
<div class="hmap-wrap">{heatmap('SP500')}</div>

<h2>NAS100 — Heatmap</h2>
<div class="hmap-wrap">{heatmap('NAS100')}</div>
...
def main():
    print('=' * 65)
    print('  SL Sweep: SP500 + NAS100 · BB 10/20/30 · SL 1.5–3.0%')
    print('=' * 65)
    print(f'\n  {len(ASSETS)} assets x {len(BB_LENS)} BB lengths x {len(SL_PCTS)} SL values = '
          f'{len(ASSETS)*len(BB_LENS)*len(SL_PCTS)} combos\n')
    ...
    for r in sorted(qualified, key=lambda x: x['pf'], reverse=True):
        print(f'  OK  {r["asset"]:7s} BB({r["bb"]}) SL {r["sl_pct"]*100:.1f}%  '
    ...
    if not qualified:
        ...
        if closest:
            print(f'  Closest: {closest["asset"]} BB({closest["bb"]}) SL {closest["sl_pct"]*100:.1f}%  '
